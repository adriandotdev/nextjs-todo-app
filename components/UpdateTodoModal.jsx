...
const UpdateTodoModal = ({ setModal, setTodos, todoToUpdate }) => {
	...
	const UpdateTodo = async (data) => {
		...
		try {
			...
			setAlert({
				is_visible: true,
				message: "Successfully updated the To-Do",
				severity: "success",
			});
			reset();
			CloseAlert();
	...
	useEffect(() => {
		setValue("title", todoToUpdate.title, { shouldValidate: true });
		setValue("priority", todoToUpdate.priority);
	}, []);
	return (
		<div
					...
					<section className="flex flex-col gap-1">
						...
						<input
							...
							className={`input input-bordered ${
								errors.title?.message ? "input-error" : "input-bordered"
							}`}
							type="text"
							name="title"
							id="title"
